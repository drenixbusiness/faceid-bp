...
const GOOGLE_SHEET_NAME = process.env.GOOGLE_SHEET_NAME || COMPANY_NAME;
...
const FORWARD_BOT_URLS = (process.env.FORWARD_BOT_URLS || '')
    .split(',').map(u => u.trim()).filter(Boolean);
const PERSONAL_BOT_URL = process.env.PERSONAL_BOT_URL || '';
...
async function sendPersonalDm(employeeId, message) {
    if (!employeeId || !PERSONAL_BOT_URL) return;
    try {
        await axios.post(`${PERSONAL_BOT_URL}/notify`, { employeeId, message });
    } catch (err) {
...
function normalizeVerifyMode(evt) {
    if (evt.FaceRect || evt.AccessControllerEvent?.FaceRect) return 'face';
    return 'fingerprint';
}
...
async function handleEvent(data, sourceIp) {
    ...
    // Only process employees registered to this company
...
app.post('/hikvision/event', upload.any(), async (req, res) => {
    ...
    try {
        ...
        // Forward raw body to all other company bots (fire-and-forget)
        for (const url of FORWARD_BOT_URLS) {
            axios.post(url, req.body, { headers: { 'x-source-ip': sourceIp } }).catch(() => { });
        }
...
app.get('/', (req, res) => res.send(`${COMPANY_NAME} attendance bot is running`));
app.get('/debug-last-event', (req, res) => res.json(lastWebhookSnapshot || { message: 'No webhook received yet' }));

app.listen(PORT, '0.0.0.0', () => {
    console.log(`✅ ${COMPANY_NAME} bot running on http://0.0.0.0:${PORT}`);
    console.log('Waiting for attendance events...\n');
    sendTelegram(`🟢 ${COMPANY_NAME} attendance bot started`);
